...
# Focusing on pdcodes (all).
# Will get all information of all i,j. Will ignore trivial data.
def get_d_pd_all(linei):

    file_path_bound = "./differentials_pd/pdcode_" + str(linei) + "/bound"
    if os.path.isfile(file_path_bound):
        text_file = open(file_path_bound, "r")
        data_bound = text_file.read()
        text_file.close()
    ...
    for i in range(i_j_bound[0], i_j_bound[1] + 1):
        for j in range(i_j_bound[2], i_j_bound[3] + 1):
            print(i,j)
            file_path_0 = "./differentials_pd/pdcode_" + str(linei) + "/" + str(i) + "_" + str(j)
            file_path_1 = "./differentials_pd/pdcode_" + str(linei) + "/" + str(i + 1) + "_" + str(j)
            data0 = ""
            data1 = ""
            ...
            # the sequence is 1.dis 2.log qubit 3.phys qubit 4.sanity check ratio
            with open("./output_pd/pdcode_" + str(linei) + "/" + str(i) + "_" + str(j), "w") as f:
                f.write(str(dis))
                f.write("\n")
                f.write(str(logical_qubit_num))
                f.write("\n")
